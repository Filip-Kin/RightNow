...
RID=$(curl -fsS -X POST -H "$AUTH" -H "Content-Type: application/json" \
  -d '{"tag_name":"android-latest","target_commitish":"main","name":"Android test build","prerelease":true,"body":"Sideloadable arm64-v8a APK (debug-signed) for the Pixel 9 Pro. Rebuilt from main on each android-apk workflow run."}' \
...
echo "Published RightNow.apk -> https://git.filipkin.com/filip/RightNow/releases/tag/android-latest"
